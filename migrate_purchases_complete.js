...
async function getItemMapping() {
  console.log('Getting item ID mappings from Supabase...');
  ...
  // Get Firebase items to create mapping
  const fbSnapshot = await firestore.collection('items').get();
  const fbItems = [];
  ...
  console.log(`Created mapping for ${itemMap.size} items`);
  return itemMap;
}
...
async function migratePurchases() {
  ...
  try {
    ...
    console.log(`Found ${purchases.length} purchases to migrate (skipped ${skippedCount})`);
