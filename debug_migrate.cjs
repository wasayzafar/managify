const admin = require('firebase-admin');
const { createClient } = require('@supabase/supabase-js');

const serviceAccount = {
  "type": "service_account",
  "project_id": "managify608",
  "private_key_id": "4c52d05e13783a5568e31ed73cc829c6d073be5e",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----",
  "client_email": "firebase-adminsdk-fbsvc@managify608.iam.gserviceaccount.com",
  "client_id": "105043392653478418673",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40managify608.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

admin.initializeApp({ credential: admin.credential.cert(serviceAccount) });
const firestore = admin.firestore();
const supabase = createClient('https://eooulfvbllitlhjzilit.supabase.co', 'sb_secret__dk16inwSwFU45qCE2sMwA_kESOiPAx');

async function debugSales() {
  console.log('Checking Firebase sales...');
  const salesSnapshot = await firestore.collection('sales').limit(5).get();
  
  console.log(`Found ${salesSnapshot.size} sales documents`);
  
  salesSnapshot.forEach(doc => {
    console.log('Sales doc:', doc.id, doc.data());
  });
  
  // Check items mapping
  const { data: supabaseItems } = await supabase.from('items').select('id, sku').limit(5);
  console.log('Supabase items sample:', supabaseItems);
  
  // Try to migrate sales with better mapping
  console.log('\nMigrating sales with item lookup...');
  const allSalesSnapshot = await firestore.collection('sales').get();
  const sales = [];
  
  for (const doc of allSalesSnapshot.docs) {
    const data = doc.data();
    console.log('Processing sale:', data);
    
    // Find item by itemId
    if (data.itemId) {
      const { data: matchedItems } = await supabase
        .from('items')
        .select('id')
        .eq('user_id', data.userId || 'default_user')
        .limit(1);
      
      if (matchedItems && matchedItems.length > 0) {
        sales.push({
          item_id: matchedItems[0].id,
          quantity: data.quantity || 1,
          date: data.date || new Date().toISOString(),
          user_id: data.userId || 'default_user',
          actual_price: data.actualPrice || 0
        });
      }
    }
  }
  
  console.log(`Prepared ${sales.length} sales for migration`);
  
  if (sales.length > 0) {
    const { error } = await supabase.from('sales').insert(sales);
    if (error) console.error('Sales migration error:', error);
    else console.log(`✅ Migrated ${sales.length} sales`);
  }
}

debugSales().catch(console.error);